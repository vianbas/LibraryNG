...
class Loan extends \yii\db\ActiveRecord
{
    ...
    {
        $copy_id = BookCopy::find()->where(['id'=>'copy_id']);
        return $this->hasOne(Book::className(), ['id' => $copy_id->book_id]);
    }
    ...
    public function getCount($id){
        $time = date("Y-m-d");
        $diff = Yii::$app->db->createCommand('SELECT datediff("'.$time.'",due_date) FROM loan where loan.id = '.$id.' ')->queryScalar();
        return $diff*200;  
    }
